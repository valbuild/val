...
2. **Each navigation panel opens with one line saying what it is.**
   `PanelHint` in `FloatingPanel`, used by Pages, Media and Data. It is inside
   the scroll area on purpose: it is for the first week, and it scrolls away
   with the first flick for everybody past that.
3. **Empty states explain rather than report.** "No pages yet" now says what a
...
4. **The empty editor lists the destinations with definitions.** It said "No
   ...
   actually has.
5. **A single media gallery opens itself.** Most projects have exactly one.
...
6. **A small site map opens itself.** With a home page at `/`, `toShellPages`
...
7. **Settings points at Account.** The split is real — Settings is
   ...
   across.
8. **A guided tour, behind a button that glows once.** Welcome, then Pages,
...
- **It can be turned off**, under Account → Workspace, defaulting to on
  (`val:tour:enabled`). That is a per-browser preference, so it sits beside the
  theme and Auto save rather than in the Settings panel, where everything is
  published content and shared by the whole team.
- **It stays reachable after the glow goes**: Quick actions, the Account panel,
  and the empty editor. Its own last step says so.
