...
/**
 * A `Description` for a story, spelled as the interesting half.
 *
 * `describePath` fills the rest from the path; a story has no store, so the
 * defaults stand in — and adding a field to `Description` then costs one line
 * here instead of one per literal.
 */
function described(
  over: Partial<Description> & Pick<Description, "title" | "pathLabel">,
): Description {
  return {
    subtitle: null,
    image: null,
      ...
      <Compare
        ...
        proposed={
          <PathHeading
            ...
            scope={<span className="text-fg-secondary-alt">Components</span>}
          />
        }
      />
      <Compare
...
const pageDescription: Description = described({
          ...
          <div key={style}>
            ...
            <div className="rounded-lg border border-border-primary bg-bg-primary p-4">
              <PathHeading
                ...
                pageUrlStyle={style}
                scope={<span className="text-fg-secondary-alt">Blogs</span>}
              />
            </div>
  ...
  render: () => (
    <div className="mx-auto max-w-2xl rounded-lg border border-border-primary bg-bg-primary">
      {FALLBACK_CASES.map(({ what, description }) => (
        ...
        >
          <div className="w-36 shrink-0 text-xs text-fg-secondary-alt">
            {what}
          </div>
          <div className="min-w-0 flex-1">
          ...
          <div className="shrink-0 text-[11px] uppercase tracking-wide text-fg-secondary-alt">
        ...
        <div key={i}>
          ...
          <div className="mb-4 rounded-lg border border-dashed border-border-primary bg-bg-primary px-4 py-3">
            <PathHeading
              ...
              scope={
                scope && <span className="text-fg-secondary-alt">{scope}</span>
              }
            />
          </div>
        </div>
