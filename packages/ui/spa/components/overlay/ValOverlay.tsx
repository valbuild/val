  ...
  useEffect(() => {
    ...
    parentRefRef.current =
      patches.length > 0
        ? {
            type: "patch" as const,
            patchBlockSha: patches[patches.length - 1],
          }
        : { type: "head" as const, headContentSha: baseSha };
  }, [patches, baseSha]);
  useEffect(() => {
    client("/patches/~", "GET", {
      query: {
      ...
      .then((res) => {
        ...
        setPatches(
          res.json.patches.map((patch) => patch.patchBlockSha) as PatchId[],
        );
  ...
  const initOnSubmit: InitOnSubmit = useCallback(
    (path) => async (callback) => {
      ...
      if (result.isOk(applyRes)) {
        ...
        parentRefRef.current = {
          type: "patch",
          patchBlockSha: applyRes.value.newPatchId,
        };
