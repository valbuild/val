/**
 * The color field with no Val plumbing at all, in a file of its own.
 *
 * Split out of `ColorField.tsx` rather than exported from it, because that file
 * also holds the store-connected field — which reaches `Preview`, and through
 * it the whole editor tree down to `ValProvider`. Anything importing the pure
 * component for its own use was taking all of that with it: the settings
 * panel, which is meant to be presentational, could no longer even be rendered
 * in a jest test.
 *
 * So the rule is the file boundary: nothing here may import a store, a
 * provider or a field. What it needs is a text input, `cn`, and the colour
 * maths from `@valbuild/core`.
 */
 ...
 *
 * Exported because the preview row in `ColorField.tsx` draws the same swatch.
 */
