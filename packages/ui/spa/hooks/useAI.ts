...
const SHOW_COMPARE_VIEW_TOOL: AITool = {
  ...
  description:
    "Navigate the user to the compare/review changes view, which shows all pending " +
    "changes side-by-side (before/after). Use when the user wants to review, compare, " +
    "or see an overview of all unpublished changes. " +
    "This view groups changes by module and shows field-level diffs.",
  parameters: {
    type: "object",
    properties: {},
    required: [],
    ...
    const handler = (message: AIServerMessage) => {
      if (message.type === "ai_streaming") {
        ...
        if (message.name === "get_all_schema") {
          ...
          navigate("/val/compare" as SourcePath);
          sendWsMessage({
            type: "ai_tool_result",
            toolCallId: message.toolCallId,
            result: { success: true },
...
- show_compare_view: open the compare/review changes view, which shows all pending changes side-by-side. Use when the user wants to review changes, see what's pending, or compare before/after versions of their edits.
