 ...
 *
 * A lookup against the project's languages, and today that is all it is. It
 * stays a named function because it is the one place the question is asked —
 * the Studio, the validation worker and the server all come here rather than
 * comparing strings themselves, so they cannot disagree about what a stored
 * value is.
 */
