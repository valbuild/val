 ...
 * asked for.
 *
 * The TanStack near-copy of `initValRsc.sourcesMemo.test.ts`. The two readers
 * are deliberate near-copies and the request scope is the one thing that
 * genuinely differs (React `cache()` there, a `WeakMap` on the `Request`
 * here) — which is exactly why the scope is injected, and why both are covered.
 *
