...
const { s, c } = initVal();
/**
 * `Internal.tanstackRouter`, not `initVal().tanstackRouter`.
 *
 ...
 *
 * The module file path is what gives a `tanstackRouter` record its pattern, so
 * it has to look like a TanStack route — `$note` is the param the test passes,
 * and the keys are the URLs it produces.
 */
const path = "/src/routes/notes.$note.val.ts" as ModuleFilePath;
...
describe("useValRoute with draft sources", () => {
   ...
   *
   * `suspend` is false on the SSR and hydration renders — `ValTanStackProvider`
