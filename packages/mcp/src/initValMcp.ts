...
import { initHandlerOptions } from "@valbuild/server";
import { createValTools } from "./tools";
import {
  VAL_SCOPE_READ,
  VAL_SCOPE_WRITE,
  type ValToolContext,
...
export function initValMcp(
  ...
  const route = opts?.route ?? "/api/val";
  const coreVersion = opts?.versions?.core ?? Internal.VERSION.core;
  if (!coreVersion) {
    throw new Error("Could not get @valbuild/core package version");
  }
  const hostVersion = opts?.versions?.next ?? VERSION;
  if (!hostVersion) {
  ...
  const setupPromise = (async () => {
    const options = await initHandlerOptions(
      route,
      { ...config, versions: { core: coreVersion, next: hostVersion } },
      config,
    );
    return {
      mode: options.mode,
      tools: createValTools(
        valModules,
        {
          ...options,
