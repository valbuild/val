...
export async function buildSnapshot(
  ...
  const manifest: SnapshotManifest = {
    ...
    versions: {
      ...
      tanstack: getVersions().tanstackVersion,
...
function generateValModules(moduleFilePaths: string[]): string {
  const imports = moduleFilePaths
    ...
    .join("\n");
  /*
   * The framework package the captured project actually has.
   *
   * A snapshot is replayed by loading this file, so naming the wrong one makes
   * it fail to resolve on a machine that has the other. `getVersions` answers
   * from what is installed; the fallback matters only for a project with
   * neither, which could not have produced a snapshot anyway.
   */
  const frameworkPackage = getVersions().tanstackVersion
    ? "@valbuild/tanstack"
    : "@valbuild/next";
...
import { modules } from "${frameworkPackage}";
...
- ${
    manifest.versions.tanstack
      ? `@valbuild/tanstack: \`${manifest.versions.tanstack}\``
